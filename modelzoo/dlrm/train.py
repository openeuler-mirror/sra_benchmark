...
def train(sess_config,
    ...
    # ckpt2pb(model, checkpoint_dir, "./saved_models")
    # read model and infer


def ckpt2pb(model, checkpoint_dir, export_dir):
    size = 39
    X = tf.placeholder(name="X", shape=[None, size], dtype=tf.float32)
    y = tf.placeholder(name="IteratorGetNext:26", shape=[None, 1], dtype=tf.float32)
    y_pred = model.probability
...
def main(tf_config=None, server=None):
    ...
    # set directory path
    model_dir = os.path.join(args.output_dir, 'model_DLRM')
    ...
    if args.mode == "evaluate":
        ...
        with open(os.path.join(args.output_dir, "dlrm_throughput_record.txt"), "a+") as f:
            f.writelines(f"dlrm: {throughput}" + "\n")
